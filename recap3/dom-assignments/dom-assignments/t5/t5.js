...
// your code here
// 1. Luodaan kartta HETI oletussijaintiin (esim. Helsinki)
const map = L.map('map').setView([60.17, 24.94], 12);

// 2. Lisätään karttapohja heti, jotta sivu ei ole tyhjä
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
  attribution: '&copy; OpenStreetMap contributors'
}).addTo(map);

// 3. Vasta nyt haetaan se oma sijainti
navigator.geolocation.getCurrentPosition((position) => {
  const myLat = position.coords.latitude;
  const myLng = position.coords.longitude;

  // Siirretään kartta käyttäjän luo
  map.setView([myLat, myLng], 13);

  // Lisätään ravintolat markerit (tämä osa pysyy samana)
  restaurants.forEach((resta) => {
    const [restLng, restLat] = resta.location.coordinates;
    const marker = L.marker([restLat, restLng]).addTo(map);
    marker.bindPopup(`<h3>${resta.name}</h3><p>${resta.address}</p>`);
  });
});
